...
def build(optimizer, noise_dim, width, height, channels):
    ...
    output_3 = discriminator_gnn(fake_image)
    ...
    return generator, discriminator_crossentropy, discriminator_sid, discriminator_gnn, gan
...
def train_step(
    ...
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc1_tape, tf.GradientTape() as disc2_tape, tf.GradientTape() as disc3_tape:
        ...
        fake_data2 = tf.cast(fake_data2, dtype=tf.float32)
        real_spectrum = tf.signal.fftshift(tf.signal.fft2d(tf.complex(real_data, 0.0)))
        fake_spectrum = tf.signal.fftshift(tf.signal.fft2d(tf.complex(fake_data2, 0.0)))
        ...
        # Preprocess the image as needed (scaling, normalization, etc.)
        fake_array = (fake_array / 256).astype(np.uint8)
        imagery = (imagery / 256).astype(np.uint8)
        ...
        print(real_graph.ndata['feat'])
        print(output.shape)
        g_loss = gnn_loss(real_graph.ndata['feat'], output)
        ...
        gen_loss = (
            ...
            + tf.cast(sr_loss, tf.float32) * tf.cast(spectral_weight, tf.float32)
            + tf.cast(g_loss, tf.float32) * tf.cast(temporal_weight, tf.float32)
        )
    ...
    return gen_loss, disc1_loss, disc2_loss, disc3_loss
...
def train(
    ...
    gnn.load_state_dict(torch.load('/Users/felipealencar/Desktop/multispectral_generative_models/gnn/trained_gnn_model.pth'))
    # Set the model in evaluation mode
    gnn.eval()
    
    for epoch in tqdm(range(epochs)):
        for _ in tqdm(range(steps)):
            gen_loss, disc1_loss, disc2_loss, disc3_loss = train_step(
                X_train,
                generator,
                gnn,
                discriminator1,
        ...
        description = f"EPOCH: {epoch + 1} Generator Loss: {gen_loss:.4f} Discriminator 1 Loss: {disc1_loss:.4f} Discriminator 2 Loss: {disc2_loss:.4f} Discriminator 3 Loss: {disc3_loss:.4f}"
